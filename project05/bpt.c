...
queue* q;
 ...
 int db_fd;
int begin_tx(){
    printf("한학기동안 정말 수고하셨습니다! 완성하지 못했지만 방학이 되면 저도 한번 완성해 보려 합니다!");
    return 0;
}

int end_tx(){
    printf("저의 작은 db를 위해 도와주신 모든 분들께 감사드립니다! 연말 잘 보내세요!!");
    return 0;
}
...
pagenum_t file_alloc_page(){
	...
	for(i=9; i>=0; i--){
		...
		c->parent=headerP->free_off;//̶ parent  ׳ next free page 
...
pagenum_t make_node( void ) {
	...
	printf("make_made - page num is %d\n",pagenum);
	free(tempP);
	return pagenum;
}

pagenum_t make_leaf( void ) {
printf("make_leaf - try make leaf\n");
    pagenum_t pagenum = make_node();
printf("make_leaf - node made\n");
    page_t * c = (page_t *) calloc(1,PAGESIZE);
    file_read_page(pagenum,c);
    c->is_leaf = true;
    file_write_page(pagenum,c);
    free(c);
printf("retured page %lld\n",pagenum);
    return pagenum;
}
...
for (i = 0; i < leafP->num_keys + 1; i++) {
       printf("%d : %s \n",temp_keys[i],temp_value[i]);
}
